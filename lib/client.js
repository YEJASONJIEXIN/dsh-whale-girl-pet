...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let { useEffect, useLayoutEffect, useRef, useState, useSyncExternalStore } = react;
		...
		const css = [
			...
			// 本 fork 新增：右下角缩放手柄（平时半透明，鼠标靠近宠物时浮现）
			'.dsh-pet-resize{position:absolute;right:2px;bottom:calc(var(--dsh-pet-size,260px) * 0.03);width:18px;height:18px;cursor:nwse-resize;pointer-events:auto;touch-action:none;opacity:.28;border-right:2px solid #4D94F5;border-bottom:2px solid #4D94F5;border-bottom-right-radius:7px;transition:opacity .15s;z-index:3}',
			'.dsh-pet-root:hover .dsh-pet-resize{opacity:.9}',
			// 拖拽调节大小期间：全局禁止选中 + 强制缩放光标
			'body.dsh-pet-resizing,body.dsh-pet-resizing *{cursor:nwse-resize !important;user-select:none !important}',
			...
			// 尺寸全部走 --wb-scale（由桌宠大小换算），所以桌宠变大按钮跟着变大；
			// 平时隐藏，鼠标移到桌宠身上才浮现（见下面的 hover 规则）。
			'.wb-stack{position:absolute;top:calc(60px * var(--wb-scale,1));left:calc(-52px * var(--wb-scale,1));z-index:3;pointer-events:auto;display:flex;flex-direction:column;gap:calc(8px * var(--wb-scale,1));align-items:flex-start;opacity:0;visibility:hidden;transform:translateX(-4px);transition:opacity .18s ease,transform .18s ease,visibility .18s}',
			'.wb-stack-right{left:auto;right:calc(-52px * var(--wb-scale,1));align-items:flex-end;transform:translateX(4px)}',
			// 鼠标移入桌宠（含按钮区）时浮现；正在查询/看板打开时保持可见
			'.dsh-pet-root:hover .wb-stack,.wb-stack:hover,.wb-stack.is-active{opacity:1;visibility:visible;transform:translateX(0)}',
			// 触屏没有 hover：这种情况下按钮常显，否则点不到
			'@media (hover:none){.wb-stack{opacity:1;visibility:visible;transform:none}}',
			'.wb-btn{width:calc(38px * var(--wb-scale,1));height:calc(38px * var(--wb-scale,1));border-radius:50%;border:1px solid #cfe4ff;background:rgba(255,255,255,.95);box-shadow:0 3px 12px rgba(30,60,120,.25);cursor:pointer;font-size:calc(17px * var(--wb-scale,1));line-height:1;display:flex;align-items:center;justify-content:center;transition:transform .15s;color:#1c2b4a;padding:0}',
			...
			'.dsh-cost-panel{position:fixed;z-index:1100;box-sizing:border-box;width:max-content;min-width:min(280px,calc(100vw - 24px));max-width:min(440px,calc(100vw - 24px));padding:16px;border:0;border-radius:12px;background:var(--dsw-specific-menu);--dsw-elevation-stroke-color:var(--dsw-alias-border-l1);box-shadow:var(--dsw-elevation-prominent);font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary);cursor:default}',
			...
			// ================================================================
			'.dsh-pet-dash-panel{position:fixed;z-index:1100;box-sizing:border-box;width:min(520px,calc(100vw - 24px));padding:14px 16px 12px;border:0;border-radius:14px;background:var(--dsw-specific-menu);--dsw-elevation-stroke-color:var(--dsw-alias-border-l1);box-shadow:var(--dsw-elevation-prominent);font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary);cursor:default}',
		...
		// 本 fork 新增：共享设置总线
		// ----------------------------------------------------------------------------
		// 桌宠（Pet）挂在 shell.overlay，设置面板挂在 settings.section，两者是互不相识的
		// 兄弟节点；但「桌宠大小」要在设置面板里改、桌宠本体立刻跟着变。所以这里放一个
		// 模块级的小 store：轮询 /settings /state 的组件写它，任何消费者订阅它。
		// React 的 useSyncExternalStore 需要 getSnapshot 返回稳定引用，
		// 因此只在值真的变化时才替换对象（避免无限重渲染）。
		// ============================================================================
		...
		const settingsStore = {
			...
			set(next) {
				...
				for (const key of Object.keys(next)) {
					if (settingsSnapshot[key] !== next[key]) { changed = true; break; }
				}
				if (!changed) return;
				settingsSnapshot = Object.assign({}, settingsSnapshot, next);
				for (const listener of settingsListeners) {
					try { listener(); } catch { /* 单个订阅者出错不影响其他 */ }
				}
			},
		};

		// ============================================================================
		// 本 fork 新增：任务播报语音（角色音色）
		// ----------------------------------------------------------------------------
		// 两级引擎：
		//   1) 角色语音（默认）：把整句 POST 给宿主的 /api/whale-pet/say，宿主用 Edge
		//      神经网络语音合成 MP3 回来，这里用 <audio> 播 —— 这样才能有"胡桃/派蒙/
		//      钟离"那种角色感（浏览器内建语音只有系统装的那一两个音色）。
		//   2) 系统语音兜底：宿主合成失败（断网/服务抖动/未启用）时，退回浏览器内建
		//      的 speechSynthesis 念 system 文本。这样"念不出角色音"也绝不会变成"没声音"。
		// 两条路都受 voiceEnabled 总开关控制。
		// ============================================================================
		const SAY_API = '/api/whale-pet/say';
		/** 系统语音兜底用的发音偏好（角色音色走宿主，不用这些）。 */
		const SYSTEM_VOICE_PREFER = ['xiaoxiao', 'huihui', 'yaoyao', 'female', '女'];
		...
		function pickSystemVoice() {
			try {
				...
				const zh = voices.filter((v) => /^zh/i.test(String(v.lang || '')));
				const pool = zh.length > 0 ? zh : voices;
				for (const key of SYSTEM_VOICE_PREFER) {
					const hit = pool.find((v) => String(v.name || '').toLowerCase().indexOf(key) >= 0);
					if (hit) return hit;
				}
				return pool[0] || null;
			} catch {
		...
		/** 用角色音色念一句：宿主合成 MP3 → 播放；失败退回系统语音。 */
		async function speakCharacter(script, character, fallbackText) {
			try {
				const response = await fetch(SAY_API, {
					method: 'POST',
					headers: { 'content-type': 'application/json' },
					body: JSON.stringify({ text: script, character }),
				});
				const data = await response.json();
				if (!data || !data.ok || !data.audio) throw new Error((data && data.error) || '合成失败');
				const audio = new Audio('data:' + (data.mime || 'audio/mpeg') + ';base64,' + data.audio);
				audio.volume = 1;
				await audio.play();
			} catch (error) {
				// 角色语音这条路不通：退回系统语音，别让播报变成没声音
				speakSystem(fallbackText || script);
			}
		}

		/**
		 * 播报入口。payload 是宿主给的 speak 对象：
		 *   { script, system, character }（旧格式是纯字符串，这里也兼容）
		 * settings.voiceEnabled === false 时完全静音。
		 */
		function speakAnnouncement(payload, settings) {
			const s = settings || settingsStore.get() || {};
			if (s.voiceEnabled === false) return;
			const item = typeof payload === 'string' ? { script: payload, system: payload } : (payload || {});
			const script = String(item.script || item.system || '').trim();
			const system = String(item.system || script).trim();
			if (!script && !system) return;
			// 设置里明确选了"只用系统语音"，就不去打宿主接口
			if (s.voiceEngine === 'system') { speakSystem(system); return; }
			speakCharacter(script, item.character || s.voiceCharacter, system);
		}

		// 有些浏览器（尤其 Chromium）语音列表是异步加载的，首次是空数组；这里预热一次，
		// 列表到位后 triggers voiceschanged，下一次播报就能挑到中文音色。
		try {
		...
		/**
		 * 语音解锁：浏览器要求"页面先有过用户交互"才允许出声（内建语音与 <audio> 都算）。
		 * 在第一次 pointerdown / keydown 时念一条空语句（volume 0，听不见）+ 播一段
		 * 静音音频，把两条路都预热好。
		 */
		...
		try {
			if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
				...
				window.addEventListener('keydown', unlockSpeech, { once: true, passive: true });
			}
		} catch { /* 忽略 */ }
...
/** 读取一次看板数据；卸载/重新请求时中断上一次。 */
		...
		// ============================================================================
		// 本 fork 新增：桌宠大小的取值工具
		//   宿主 Config.size 允许 40-400；拖拽把手再放宽到 600，方便想要"大号桌宠"的人。
		// ============================================================================
		const PET_SIZE_MIN = 80;
		const PET_SIZE_MAX = 600;
		/**
		 * 设置订阅 hook。优先用 React 18 的 useSyncExternalStore（官方并发安全写法）；
		 * 万一宿主外壳给的 React 里没有这个导出，就退回 useState + useEffect 订阅
		 * ——功能一样，只是极端情况下可能多渲染一帧。
		 */
		function useSettings() {
			...
			const [value, setValue] = useState(settingsStore.get);
			useEffect(() => settingsStore.subscribe(() => setValue(settingsStore.get())), []);
			return value || {};
		}
		 ...
		 */
		function Pet({ config }) {
			// ---- 本 fork：订阅共享设置总线（桌宠大小 / 语音开关等即时生效） ----
			const liveSettings = useSettings();
			// ---- 从 config 读取参数（patch 配置；实时代码走 liveSettings） ----
			const size = clampPetSize(liveSettings.size !== undefined ? liveSettings.size : (config && config.size));
			...
			const [balanceLoading, setBalanceLoading] = useState(false); // 余额按钮加载中（按钮堆保持可见）
					...
					try {
						...
						for (const item of items) {
							...
							if (item.type === 'mood') applyMood(item.mood);
								...
								// 本 fork：先念出来（宿主已按"完成/中断/失败 + 任务名"生成整句，
								// 并带上该用哪个角色音色）
								if (item.speak) speakAnnouncement(item.speak, data && data.settings);
								playNotice(item.ok, item);
							}
						}
					} catch {
						// 网络波动忽略，下个周期重试
			...
			// ============================================================================
			// 本 fork 新增：桌宠大小调整
			//   · 设置面板里改（滑块 / 数字框 / 预设档 / 重置）
			//   · 宠物右下角拖拽把手（按住往右下拉变大、往左上推变小）
			//   两条路都写同一个设置键 size（写进宿主 settings.yaml，重启不丢）。
			// ============================================================================
			const saveSize = (value) => {
				const next = clampPetSize(value);
				// 本地先落值：拖拽时不等网络往返，画面立刻跟手
				settingsStore.set({ size: next });
				settingsRef.current = Object.assign({}, settingsRef.current || {}, { size: next });
				fetch('/api/whale-pet/settings', {
					method: 'POST',
					headers: { 'content-type': 'application/json' },
					body: JSON.stringify({ ops: [{ op: 'set', path: ['size'], value: next }] }),
				}).catch(() => {});
			};
			const onSizePointerDown = (event) => {
				event.preventDefault();
				event.stopPropagation();
				markActivity(); // 调整大小也算互动：别在这时候睡着
				const start = { x: event.clientX, y: event.clientY, size: size };
				const target = event.currentTarget;
				// 用"对角方向的投影"当拖动距离：往右下拖为正，往左上推为负 —— 手柄贴哪都顺手
				const move = (e) => {
					const dx = e.clientX - start.x;
					const dy = e.clientY - start.y;
					saveSize(start.size + (dx + dy) / 2);
				};
				const up = () => {
					window.removeEventListener('pointermove', move);
					window.removeEventListener('pointerup', up);
					window.removeEventListener('pointercancel', up);
					document.body.classList.remove('dsh-pet-resizing');
				};
				document.body.classList.add('dsh-pet-resizing');
				window.addEventListener('pointermove', move);
				window.addEventListener('pointerup', up);
				window.addEventListener('pointercancel', up);
				if (target && typeof target.setPointerCapture === 'function') {
					try { target.setPointerCapture(event.pointerId); } catch { /* 忽略 */ }
				}
			};

			// 渲染树：root > [任务气泡, 打字气泡, stage > video A/B]
			// A 初始带 is-front（显示），B 隐藏待命
			// 按钮随桌宠大小等比缩放：以默认 260px 为 1 倍基准，限制在 0.75~1.7 之间，
			// 免得小桌宠的按钮糊成一团、大桌宠的按钮大到挡脸。
			const wbScale = Math.min(1.7, Math.max(0.75, size / 260));
			return h('div', {
				ref: rootRef,
				className: 'dsh-pet-root',
				'data-corner': corner,   // CSS 决定默认角落
				'data-facing': facing,   // CSS 决定是否镜像
				style: Object.assign({ '--dsh-pet-size': size + 'px', '--wb-scale': String(wbScale) }, rootStyle),
				children: [
					...
					// 平时隐藏、鼠标移入桌宠才浮现（CSS 的 .dsh-pet-root:hover），
					// 正在查询 / 看板打开 / 正在拖拽宠物时保持可见，避免"点了一半按钮跑了"。
					// 按钮尺寸随桌宠大小缩放（--wb-scale）。
					// 本 fork：📊 按钮用 forwardRef 把 ref 落到真实 <button> 上，
					// 作为看板弹层的锚点（包裹 span 是 display:contents，没有盒子不能当锚点）
					react.createElement('div', {
						className: 'wb-stack'
							+ (((settingsRef.current || {}).buttonSide === 'right') ? ' wb-stack-right' : '')
							+ ((weatherLoading || balanceLoading || dashOpen || dragging) ? ' is-active' : ''),
					},
						react.createElement(WeatherButton, { loading: weatherLoading, onClick: handleWeather }),
						react.createElement(BalanceButton, {
							onResult: (r) => showBubble(r),
							onPlay: () => playAction('翻钱包'),
							onBusy: setBalanceLoading,
						}),
					...
					// 本 fork 新增：右下角缩放手柄（拖拽调整桌宠大小，松手即保存）
					!dragging ? h('div', {
						className: 'dsh-pet-resize',
						title: '拖动调整桌宠大小（' + size + 'px）',
						role: 'slider',
						'aria-label': '桌宠大小',
						'aria-valuemin': PET_SIZE_MIN,
						'aria-valuemax': PET_SIZE_MAX,
						'aria-valuenow': size,
		...
		function BalanceButton({ onResult, onPlay, onBusy }) {
			const [loading, setLoading] = useState(false);
			const onClick = async () => {
				if (loading) return;
				setLoading(true);
				// 让按钮堆保持可见：查询期间鼠标可能已经移开，按钮不能跟着消失
		...
		// 本 fork 新增：角色音色列表（宿主 /api/whale-pet/voices 提供）。
		// 拿不到时用这份最小兜底，保证设置面板不会空着。
		...
		function PetSettingsSection() {
			...
			// 角色音色列表（宿主拉取 Edge 语音清单）
			const [characters, setCharacters] = useState(FALLBACK_CHARACTERS);
			// Edge 底层语音清单（「自定义」下拉用）
			const [edgeVoices, setEdgeVoices] = useState([]);
			useEffect(() => {
				fetch('/api/whale-pet/voices')
					.then((r) => r.json())
					.then((data) => {
						if (data && Array.isArray(data.characters) && data.characters.length > 0) setCharacters(data.characters);
						if (data && Array.isArray(data.voices)) setEdgeVoices(data.voices);
					})
					.catch(() => {});
			}, []);
			const load = async () => {
			...
			useEffect(() => { load(); }, []);
			// 本 fork：桌宠大小的即时预览值。拖滑块时先改本地，松手（onChange 尾）才写盘，
			// 这样拖动过程中宠物跟着变、又不会每个像素都发一次请求。
			const [sizeDraft, setSizeDraft] = useState(null);
			// 用 ref 镜像一份：window 级 pointerup 回调里要读到最新草稿（闭包里的 state 是旧的）
			const sizeDraftRef = useRef(null);
			const setDraft = (value) => { sizeDraftRef.current = value; setSizeDraft(value); };
			const persistSizeDraft = () => {
				const draft = sizeDraftRef.current;
				if (draft === null || draft === undefined) return;
				sizeDraftRef.current = null;
				setDraft(null);
				save([{ op: 'set', path: ['size'], value: draft }]);
			};
			// 手指/鼠标在滑块外面松开也要落盘（元素上的 pointerup 那时不会触发）
			useEffect(() => {
				const onUp = () => persistSizeDraft();
			...
			// 本 fork：true=开 / false=关 的"选择性开关"（默认开，如漫游 / 语音 / 看板补扫）
			const optOutToggle = (path, labelOn, labelOff) => react.createElement('button', {
				style: Object.assign({}, S_BTN, s[path] !== false ? S_BTN_ON : null),
				onClick: () => save([{ op: 'set', path: [path], value: s[path] === false }]),
			}, s[path] !== false ? labelOn : labelOff);
			// 本 fork：桌宠大小 —— 拖动时本地预览（立即变），松手才写盘
			const currentSize = clampPetSize(sizeDraft !== null ? sizeDraft : s.size);
			const sizeSlider = react.createElement('input', {
				type: 'range',
				min: PET_SIZE_MIN,
				max: PET_SIZE_MAX,
				step: 10,
				value: currentSize,
				style: { width: 200, accentColor: '#4D94F5' },
				onChange: (e) => {
					const next = clampPetSize(e.target.value);
					setDraft(next);
					settingsStore.set({ size: next }); // 实时预览：不等写盘
				},
				// 拖动结束 / 键盘调整后落盘一次（用 window 上的 pointerup 更可靠：
				// 手指在滑块外面松开时，元素上的 pointerup 是不触发的）
				onPointerUp: () => persistSizeDraft(),
				onBlur: () => persistSizeDraft(),
				onKeyUp: () => persistSizeDraft(),
			});
			const sizePresets = react.createElement('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' } },
				[[180, '小'], [260, '中'], [360, '大'], [480, '特大']].map(([v, label]) => react.createElement('button', {
					key: String(v),
					style: Object.assign({}, S_BTN, currentSize === v ? S_BTN_ON : null),
					onClick: () => { setDraft(null); settingsStore.set({ size: v }); save([{ op: 'set', path: ['size'], value: v }]); },
				}, label + ' ' + v)),
				react.createElement('button', {
					style: S_BTN,
					onClick: () => { setDraft(null); settingsStore.set({ size: 260 }); save([{ op: 'set', path: ['size'], value: 260 }]); },
				}, '重置'));
			...
			return react.createElement(SettingsSectionBoundary, null,
				react.createElement('div', { style: { maxWidth: 520, padding: '4px 0 20px' } },
					...
					// 本 fork 新增：桌宠大小（滑块 / 预设档 / 也可直接拖宠物右下角把手）
					react.createElement('div', { style: S_ROW },
						react.createElement('span', null, '桌宠大小（' + currentSize + 'px，也可拖宠物右下角把手）'),
						react.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' } },
							sizeSlider,
							react.createElement('button', {
								style: S_BTN,
								title: '按当前大小减一档',
								onClick: () => { const v = clampPetSize(currentSize - 40); setDraft(null); settingsStore.set({ size: v }); save([{ op: 'set', path: ['size'], value: v }]); },
							}, '－'),
							react.createElement('button', {
								style: S_BTN,
								title: '按当前大小加一档',
					...
					react.createElement('div', { style: S_ROW }, '大小预设', sizePresets),
					// 本 fork 新增：任务播报语音（角色音色）
					react.createElement('div', { style: S_ROW }, '任务播报语音（完成 / 中断 / 失败，含任务名）', optOutToggle('voiceEnabled', '开', '关')),
					react.createElement('div', { style: S_ROW }, '播报引擎', react.createElement('div', { style: { display: 'flex', gap: 6 } },
						[['edge', '角色语音（推荐）'], ['system', '系统语音']].map(([v, label]) => react.createElement('button', {
							key: v,
							style: Object.assign({}, S_BTN, (s.voiceEngine === undefined ? 'edge' : s.voiceEngine) === v ? S_BTN_ON : null),
							onClick: () => save([{ op: 'set', path: ['voiceEngine'], value: v }]),
						}, label)))),
					// 角色音色：点一下就试听，顺带保存
					react.createElement('div', { style: Object.assign({}, S_ROW, { alignItems: 'flex-start' }) },
						react.createElement('span', { style: { flex: 'none', paddingTop: 4 } }, '角色音色'),
						react.createElement('div', { style: { display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' } },
							(characters.length > 0 ? characters : FALLBACK_CHARACTERS).map((c) => react.createElement('button', {
								key: c.id,
								title: c.note ? c.name + '：' + c.note : c.name,
								style: Object.assign({}, S_BTN, (s.voiceCharacter === undefined ? 'hutao' : s.voiceCharacter) === c.id ? S_BTN_ON : null),
								onClick: () => {
									// 立即试听 + 保存：选音色本来就是"听着挑"的过程
									speakAnnouncement(
										{ script: '「帮你修一下登录的 bug」完成啦，收工！', system: '「帮你修一下登录的 bug」完成啦，收工！', character: c.id },
										Object.assign({}, s, { voiceEnabled: true, voiceEngine: 'edge', voiceCharacter: c.id }),
									);
									save([{ op: 'set', path: ['voiceCharacter'], value: c.id }]);
								},
							}, c.name)))),
					react.createElement('div', { style: S_ROW }, '试听当前音色', react.createElement('button', {
						style: S_BTN,
						onClick: () => speakAnnouncement(
							{ script: '「帮你修一下登录的 bug」完成啦，收工！', system: '「帮你修一下登录的 bug」完成啦，收工！', character: s.voiceCharacter },
							Object.assign({}, s, { voiceEnabled: true, voiceEngine: 'edge' }),
						),
					}, '🔊 念一句')),
					// 自定义：底层音色 + 音调 + 语速。预设不够像时，自己往想要的方向调。
					react.createElement('div', { style: Object.assign({}, S_ROW, { alignItems: 'flex-start' }) },
						react.createElement('span', { style: { flex: 'none', paddingTop: 4 } }, '自定义（覆盖上面预设）'),
						react.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end' } },
							react.createElement('select', {
								style: Object.assign({}, S_INPUT, { width: 220 }),
								value: s.voicePitchVoice || '',
								onChange: (e) => save([{ op: 'set', path: ['voicePitchVoice'], value: e.target.value }]),
							},
								react.createElement('option', { value: '' }, '（用角色预设的音色）'),
								(edgeVoices.length > 0 ? edgeVoices : []).map((v) => react.createElement('option', {
									key: v.shortName,
									value: v.shortName,
								}, v.shortName.replace('Neural', '') + (v.personalities.length ? ' · ' + v.personalities.join('/') : '')))),
							react.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center' } },
								react.createElement('span', { style: { fontSize: 11, color: '#55627c' } }, '音调'),
								react.createElement('input', {
									type: 'text', style: Object.assign({}, S_INPUT, { width: 70 }),
									defaultValue: s.voicePitch || '+60Hz', key: 'pitch' + String(s.voicePitch || ''),
									onBlur: (e) => save([{ op: 'set', path: ['voicePitch'], value: String(e.target.value).slice(0, 8) }]),
								}),
								react.createElement('span', { style: { fontSize: 11, color: '#55627c' } }, '语速'),
								react.createElement('input', {
									type: 'text', style: Object.assign({}, S_INPUT, { width: 70 }),
									defaultValue: s.voiceRate || '+15%', key: 'rate' + String(s.voiceRate || ''),
									onBlur: (e) => save([{ op: 'set', path: ['voiceRate'], value: String(e.target.value).slice(0, 8) }]),
								})))) ,
		...
		function CostDialog(props) {
			...
			react.createElement('div', { className: 'dsh-cost-title' },
				...
				react.createElement('span', { className: 'dsh-cost-total' }, fmtCost(view.totalCny))),
		...
		function CostPill(props) {
			...
			if (view.pricedRequests + view.unpricedRequests === 0) return null;
			const peak = isPeakNow();
			const period = peak ? t('dialog.periodPeak') : t('dialog.periodOffPeak');
			const totalText = fmtCost(view.totalCny);
			...
			return react.createElement('div', { className: 'dsh-cost-root' },
				react.createElement('span', { ref: anchorRef, className: 'dsh-cost-anchor' },
					...
					open && (costContainer === null
						? react.createElement(CostDialog, { view, t, panelRef, pos })
						: createPortal(react.createElement(CostDialog, { view, t, panelRef, pos }), costContainer))));
		}
		...
		function TurnCostPill(props) {
			...
			if (row === undefined || !(row.totalCny > 0)) return null;
			const totalText = fmtCost(row.totalCny);
			const peak = isPeakNow();
			const period = peak ? t('dialog.periodPeak') : t('dialog.periodOffPeak');
			...
			react.createElement('div', { className: 'dsh-cost-rule', 'aria-hidden': true }),
			react.createElement('dl', { className: 'dsh-cost-details' },
				costRow(t('dialog.cacheRead'), fmtCost(row.cacheReadCny), 'hit'),
				costRow(t('dialog.miss'), fmtCost(row.missCny), 'miss'),
				costRow(t('dialog.output'), fmtCost(row.outputCny), 'out'),
				costRow(t('dialog.peak'), fmtCost(row.peakCny), 'peak'),
				costRow(t('dialog.offPeak'), fmtCost(row.offPeakCny), 'off')),
